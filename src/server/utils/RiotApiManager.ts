...
export class RiotApiManager {
  ...
  private async waitForRateLimit(): Promise<void> {
    ...
    if (this.requestCount >= this.RATE_LIMIT_PER_2MINUTES) {
      ...
      console.log(
        `RiotApiManager: 2分間のRate Limitに達しました。${waitTime}ms待機します`
      );
  ...
  /**
   * Spectator API - Featured Games（現在進行中の注目試合一覧）
   */
  async getFeaturedGames(): Promise<any> {
    const url = `https://jp1.api.riotgames.com/lol/spectator/v5/featured-games?api_key=${this.apiKey}`;
    return this.request<any>(url);
  }

  /**
   * Summoner API - サモナー名からサモナー情報取得
   */
  async getSummonerByName(name: string): Promise<any> {
    const url = `https://jp1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${encodeURIComponent(
      name
    )}?api_key=${this.apiKey}`;
    return this.request<any>(url);
  }
  ...
  /**
   * Account API - PUUID から Riot ID (gameName/tagLine) 取得
   */
  async getAccountByPuuid(puuid: string): Promise<any> {
    const url = `https://asia.api.riotgames.com/riot/account/v1/accounts/by-puuid/${encodeURIComponent(
      puuid
    )}`;
    return $fetch<any>(url, {
      headers: {
