...
import {
  OpenRouterClient,
  type PreMatchAdvicePayload,
} from "~/server/utils/OpenRouterClient";
...
export default defineEventHandler(async (event) => {
  ...
  const hasNames = [...myTeam, ...enemyTeam].every(
    (p: any) => typeof p?.championName === "string" && p.championName.length > 0
  );
  if (
    !hasNames ||
    typeof myChampion?.championName !== "string" ||
    !myChampion.championName.length
  ) {
    throw createError({
      statusCode: 400,
      message: "championName が必要です（IDではなく名称を送ってください）",
    });
  }
  ...
  try {
    ...
    const payload: PreMatchAdvicePayload = {
      gameId,
      gameInfo,
      myChampion,
      myTeam,
    ...
    const json = await client.generatePreMatchAdvice(payload);
    return json;
  } catch (e: any) {
