...
function calculateAverageRank(participants: any[]): {
  tierScore: number;
  displayRank: string;
} {
  const rankedParticipants = participants.filter(
    (p) => p.rank && p.rank.tier && p.rank.rank
  );
...
export default defineEventHandler(
  async (
    event
  ): Promise<LiveMatchDetail | { isInGame: false; message: string }> => {
    try {
      // リクエストボディを取得
      ...
      console.log(
        "Debug - 進行中試合情報を取得します, PUUID:",
        puuid.substring(0, 20) + "..."
      );
        ...
        async (participant: any, index: number) => {
          try {
            console.log(
              `Debug - ${
                index + 1
              }/10 プレイヤー PUUID: ${participant.puuid?.substring(
                0,
                20
              )}... のランク情報取得開始`
            );
            ...
            const summonerInfo = await riotApi.getSummonerByPuuid(
              participant.puuid
            );

            console.log(
              `Debug - プレイヤー ${index + 1} のランクAPI応答:`,
              rankInfo?.length,
              "件"
            );
            ...
            console.warn(
              `Debug - プレイヤー ${index + 1} のランク情報取得失敗:`,
              {
                status: error.status,
            ...
            try {
              ...
              console.warn(
                `Debug - プレイヤー ${index + 1} のサモナー情報取得も失敗`
              );
            }
      ...
      const enemyTeam = participantsWithRank.filter(
        (p) => p.teamId !== myTeamId
      );
      ...
      const markWinRateExtremes = (team: any[]) => {
        const candidates = team
          ...
          .filter(({ rank }) => !!rank && rank!.wins + rank!.losses > 0) as {
          p: any;
          idx: number;
          rank: RankInfo;
        }[];
